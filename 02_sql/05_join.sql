...
-- 매번 하나하나 해야해서 너무 많은 셀렉이 필요하다. > 어떻게 하면 한 번에 할 수 있을까?
...
select d.dept_id, d.dept_name, avg(e.salary) "급여평균"
from emp e join dept d on e.dept_id = d.dept_id
...
-- from   emp e right join dept d on e.dept_id = d.dept_id -- 오른쪽 중심 dept
from   emp e left join dept d on e.dept_id = d.dept_id -- 왼쪽 중심 emp 있으면 정보 없어도 dept 출력
...
select d.dept_id, d.dept_name, count(emp_id) "직원 수" -- 직원 수 구할 때 내가 세려고 하는 테이블의 primary key로 count하는게 실수를 줄인다.
...
select e.emp_id, e.emp_name "직원이름", m.emp_name "상사이름", date_format(e.hire_date, '%Y/%m/%d') "입사일"
...
select e.emp_id, e.emp_name "직원 이름", j.job_title, e.salary, e.hire_date"직원 입사일", m.emp_name "상사이름", m.hire_date"상사 입사일", d.dept_name, d.loc
from emp e left join job j on e.job_id = j.job_id
