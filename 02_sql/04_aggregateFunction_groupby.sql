...
select count(distinct dept_name) from emp; -- 고유값 개수 확인 가능(null 제외)
select count(distinct ifnull(dept_name, 'a')) from emp; -- null 값도 포함해서 고유값 count
...
-- comm_pct 평균
select avg(comm_pct), -- comm_pct가 있는 35명 기준 평균(null 제외 계산)
	   avg(ifnull(comm_pct, 0)) -- 전체 직원에 대한 평균(null -> 0 변경한 뒤 평균)
from emp;
...
/* select count(*)
from emp
group by salary <= 10000, salary > 10000; */

select case when salary <= 10000 then '10000 이하'
		    else '10000 초과'  end '급여기준',
	   count(*) '직원 수'
from emp
