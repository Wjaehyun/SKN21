...
-- EMP 테이블의 업무(job) 어떤 값들로 구성되었는지 조회. - 동일한 값은 하나씩만 조회되도록 처리. (distinct)
...
select emp_id as 직원ID, # emp_id 컬럼의 조회결과를 직원 ID 컬럼으로 보여줘
	   upper(emp_name) as 직원이름, # 대문자
       hire_date as 입사일,
       salary as 급여,
       dept_name as 소속부서
from emp;

-- as 생략 가능하다 , 다 별칭을 주지 않아도 된다.
-- 별칭을 " "로 감싼다(필수는 아니지만 구분을 위해서)
select emp_id, # emp_id 컬럼의 조회결과를 직원 ID 컬럼으로 보여줘
	   upper(emp_name) "emp_name", # 대문자, emp_name 안에 있는 리스트의 값에 처리해서 보여준다.
       hire_date "입사일",
       salary "급여",
       dept_name "소속부서"
from emp;
...
-- EMP 테이블에서 직원의 이름(emp_name), 급여(salary), 급여(salary)을 연봉으로 조회. (곱하기 12)ㄴ
...
select emp_id, emp_name, dept_name from emp where emp_id = 110;
 
-- EMP 테이블에서 'Sales' 부서에 속하지 않은 직원들의 ID(emp_id), 이름(emp_name),  부서명(dept_name)을 조회.
select emp_id "ID",
	   emp_name "이름",
       dept_name "부서명" 
from emp where dept_name != "Sales";
...
from emp
where emp_name like 'S%' ; -- % 글자수와 문자가 뭐가 오든 상관 없다. 
-- where emp_name like '%en'; : ~(en)으로 끝나는
-- where emp_name like "%are%"; : ~ 를 포함한
-- where emp_name like 'K%abc'; : K로 시작, abc로 끝나는
...
where emp_name like '%&%%' escape '&'; -- escape '특수문자', 특수문자 뒤에 % 또는 _ 조회할 값을 표시 
...
-- where hire_date > '2004-05-31; : 2004년 5월 31일 이후 입사한 직원 조회
...
select emp_name, job, dept_name, hire_date
from emp
